 ...
 * negative offset used by OOB() to range-check the next continuation byte. */
...
/* The UTF-8 DFA transition table is musl's shared __fsmu8 (src/multibyte/internal.c,
 * built from the fork) -- one copy for the whole package, indexed here and by the
 * fork's mbtowc.c. Built from the same musl R()/state macros, so byte-identical to
 * the table this TU used to carry file-locally. */
extern const uint32_t __fsmu8[];
...
/* mbtowc is musl's DFA VERBATIM + one folding `sizeof(wchar_t)<4` guard for the
 * supplementary case -> lives in the musl fork (src/multibyte/mbtowc.c). It is
 * self-contained (needs no surrogate helpers), unlike the restartable string
 * engines below, which weave the UTF-16 surrogate logic and stay here. */
