// fsops_darwin.go — the per-OS fs bridges on macOS: fdatasync (no such syscall on
// darwin, so fall back to fsync, matching musl on systems that lack it) and isatty
// (probe with the darwin TIOCGETA termios ioctl).
//
//go:build darwin
...
func rawIsatty(fd int32) int32 {
	...
	_, _, e := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd),
		0x40487413 /*TIOCGETA*/, uintptr(unsafe.Pointer(&t[0])))
	if e == 0 {
		return 1
	}
	if e == syscall.EBADF {
